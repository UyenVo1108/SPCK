...
function showdata(data) {
  ...
  data.forEach((item) => {
    ...
    <div class="card">
      <img data-id="${item.id}" id="imgProduct" src="./images/${
      item.image
    }" class="card-img-top product-image" alt="..." height="280"/>
      <div class="card-body">
        <h5 id="nameProduct" class="text-dark card-title">${item.name}</h5>
        <div id="categoryProduct" class=" fs-5 category">${item.category}</div>
        <div class="price mt-2">
          <span id="priceNewProduct" class=" fs-5 price-sale">${item.price.toLocaleString()}Đ</span>
          <del id="priceOldProduct" class=" fs-5 text-black-50 price-old">${item.priceold.toLocaleString()}Đ</del>
        </div>
        <button id="addcart" class="btn btn-success addcart mt-2">Mua ngay</button>
      </div>
    </div>
  </div>`;
  });
